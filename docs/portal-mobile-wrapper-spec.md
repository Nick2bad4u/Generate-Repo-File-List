# Portal Mobile Wrapper — Plan

> Working name: **bea-portals-mobile**
> Status: Draft v0.1
> Targets: iOS + Android wrappers for `portal-uk`, `portal-us-ca`, `BEA_Creator_Portal`
...
1. **Push notifications** — payout posted, LIVE schedule reminder, new training module, gift milestone hit
2. **Home screen presence** — the portal is one tap away, not three navigation steps
3. **App store credibility** — for an agency, having an iOS+Android app signals legitimacy to new creators

A Flutter wrapper around the existing web portals is the fastest path to all three without rebuilding the UI natively. The forked `full_web_converter_To_flutter_App` is designed for exactly this conversion.
...
### In scope (v1)
- Single Flutter app that hosts all three portals (region picker on first launch)
- Native push notifications
- Biometric / native auth on app open
- Deep linking from emails / SMS into specific portal screens
...
```
┌─────────────────────────────────────────────────────────────┐
│  FLUTTER APP (single binary, multi-region)                  │
│  - Region picker (UK / US-CA / Internal)                    │
│  - Native auth shell (biometric, secure storage)            │
│  - Push notification handler                                │
│  - Deep link router                                         │
│  - WebView host (one of: flutter_inappwebview, webview_flutter) │
└────────────────────────────┬────────────────────────────────┘
                             │ JS bridge
                             ↓
┌─────────────────────────────────────────────────────────────┐
│  WEB PORTAL (existing — portal-uk / portal-us-ca / BEA_CP)  │
│  - Detects "in BEA app" via UA string                       │
│  - Hides web-only chrome (region switcher, footer)          │
│  - Calls bridge for native features (push opt-in, biometric)│
└─────────────────────────────────────────────────────────────┘
```

The web portals stay the source of truth. The Flutter shell adds native capabilities and store distribution. Total native code stays under ~3000 LOC if scoped well.
...
- Multi-portal config (URL per region, single binary)
...
### 4.2 Web portal additions

Each portal repo needs a small "mobile mode" branch:

- Detect UA: `BEA-Mobile/1.0`
- Apply `.in-app` CSS class to hide region switcher, replace browser back with native back
...
Goal: Prove `full_web_converter_To_flutter_App` can wrap one portal with a WebView and load it on an iOS Simulator + Android Emulator.

- Clone fork
- Configure with `https://portal-uk.bea.com` (or staging URL)
- Brand splash, smoke test
- Verify session cookies persist across app launches

**Deliverable:** A `.apk` + `.app` on dev machines, demoed.

**Decision gate:** Is the WebView UX acceptable? Specifically: scrolling, keyboard handling, file pickers, camera access for screenshot uploads.
...
## 8. Cross-cutting dependency: §portal-security-audit-spec
...
- [ ] Certificate pinning for portal API calls (prevent MITM on hostile WiFi)
...
1. One single app with region picker, or three branded apps (one per portal)? *Recommend single app for v1 — simpler ops.*
...
1. Phase 0 spike — 3 days to validate WebView UX works for the actual portal screens (especially CSV/screenshot upload, which is finicky).
2. Decision gate after spike — go/no-go before committing to Phase 1.
3. If go: create `bea-portals-mobile` repo, scaffold per §3, set up TestFlight + Play Internal Testing.
4. Coordinate with `portal-security-audit-spec` work so mobile launch isn't blocked at the end by surprises from the audit.
