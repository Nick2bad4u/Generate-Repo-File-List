...
- Payout integration (Stripe, integrated per your confirmation)
...
| Stripe integration (webhooks, payouts, account flows) | Internal-only ops tools, if any |
...
- [ ] Map all third-party integrations (Stripe, TikTok API, Cloudflare, Supabase, NotebookLM if connected, etc.)
...
#### Payments (Stripe is integrated — confirmed)
...
- [ ] Connected account flow audited if using Stripe Connect for creator payouts
...
- [ ] Revoke any service-account credentials those repos held (Stripe keys, Supabase service keys, etc.) if they weren't already rotated during consolidation.

The last bullet is the security one — abandoned repos with live credentials are a common breach vector.
